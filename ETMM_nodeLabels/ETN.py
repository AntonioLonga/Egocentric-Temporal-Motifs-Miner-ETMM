...
def from_ETNS_to_ETN(s,k,meta=None):
    ...
    if not(meta==None):
        ...
        node_encoding = [s[5:][i:i+n*(len(meta_binary[0]))] for i in range(0, len(s[5:]), n*len(meta_binary[0]))]
        ego_encoding = s[2:5]
    ...
    for i in range(len(node_encoding)):
        ...
        for j in range(n):
            if not(node_encoding[i][j] == "0"):
                ...
                if not (meta ==None):
                    ETN.nodes()[str(i+1)+"_"+str(j)]["label"] = node_encoding[i][j]
...
def draw_ETNs(S,k,meta,n_per_row=5,top_k=10):
    ...
    for i in range(top_k+1):
        if len(ETNstmp) == n_per_row:
            ETNs.append(ETNstmp.copy())
            ETNstmp = []
        else:
            ETNstmp.append(from_ETNS_to_ETN(etns[i],k,meta))
    if not(len(ETNstmp)==0):
        ETNs.append(ETNstmp)

    iii = 0
    for kk in range(len(ETNs)):
        plt.figure(figsize=(15,3))
        ii = 0
        for ETN in  ETNs[kk]:
            plt.subplot(1,n_per_row,ii+1)
            plt.title("count: "+str(counts[iii]))
            iii = iii + 1
            ii = ii + 1
            ids,k = get_ids_and_k(ETN)
            pos = dict()
